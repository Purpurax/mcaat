...
vector<string> extract_sequences_from_fastq_file(const string& fastq_file_path) {
    ...
    try {
        ...
        std::cerr << "\" sequences because: " << e.what() << endl;
    }
...
optional<Jump> create_jump_from_sequence(
    ...
    optional<uint64_t> start_node_id;
    optional<uint64_t> end_node_id;
...
vector<Jump> get_jumps_from_reads(
    ...
    #pragma omp parallel num_threads(thread_count)
    {
        ...
        for (const auto& sequence : sequences) {
            const optional<Jump> jump_opt = create_jump_from_sequence(sequence, kmer_to_node_id, K);
